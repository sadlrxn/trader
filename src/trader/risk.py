...
class RiskManager:
    ...
    def size_signal(
        ...
        if risk_per_share <= Decimal("0"):
            return RiskDecision(
                approved=False, reason="Signal stop is not below entry."
            )
        ...
        risk_quantity = int(
            (risk_budget / risk_per_share).to_integral_value(rounding=ROUND_DOWN)
        )
        max_position_value = equity * self._settings.trader_max_position_notional_pct
        notional_quantity = int(
            (max_position_value / signal.entry_price).to_integral_value(
                rounding=ROUND_DOWN
            )
        )
        ...
        if quantity <= 0:
            return RiskDecision(
                approved=False, reason="Risk budget does not allow a minimum size."
            )

        return RiskDecision(
            approved=True, quantity=quantity, reason="Risk sizing approved."
        )
